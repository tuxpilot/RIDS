...
It is a home made, POC (Proof Of Concept) script, based on shell and PHP, using the GPIO (General Purpose In and Out) interfaces of the Raspberry.
It has been developped on a Raspberry Pi 3b+.
...
sudo vim /etc/boot/config.txt   =>   dtparam=spi=on
...
You will have to customize the alarm before you hope it matches the environment and your expectations
To do so, you have to get to your RIDS Web User Inteface
And click on the "Enter Management Mode" icon
Refresh the page, and it should show you some more icons.
...
Click on the "GPIO Management" icon to get in the GPIO management page
Here is the page where you set up your access points.
Every access point you connected to a GPIO needs to be set up here.
